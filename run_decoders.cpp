        ...
        if(strcmp(params[i].pipeline.c_str(),"libfreenect2") == 0)
        {
		        std::string depth_filename = "../data/";
				    ...
				    depth_filename.append(data_set);
						depth_filename.append(".bin");

				    conf_filename.append("_conf_");
				    conf_filename.append(params[i].setup_name);        
				    conf_filename.append("_");
				    conf_filename.append(data_set);
						conf_filename.append(".bin");
				
						if(!(std::ifstream(depth_filename.c_str()) || std::ifstream(conf_filename.c_str())))
						{
						  std::fstream* depth_file = new std::fstream(depth_filename.c_str(), std::ios::out | std::ios::binary | std::ios::app);
						  std::fstream* conf_file = new std::fstream(conf_filename.c_str(), std::ios::out | std::ios::binary | std::ios::app);
